...
set.seed(1234)
index <- createDataPartition(bank_balanced$Attrition_Flag , p =0.8, list = FALSE)

train.bank_balanced <- bank_balanced[index,]
test.bank_balanced <- bank_balanced[-index,]
...
bank_balanced.boost <- boosting(Attrition_Flag ~ ., data = train.bank_balanced, boos = TRUE)
# print(bank.boost)
...
bank_balanced.rf <- randomForest(Attrition_Flag ~ ., data = train.bank_balanced, ntree = 500,
                        seed=123, importance = TRUE)
...
varImpPlot(bank_balanced.rf, sort = TRUE, main = "Variable Importance")
...
#-------------------------------------------------------------------------------
# ASSESSMENT
cat("Results on whole dataset (in the sense of whole attributes!):\n")
# AdaBoost
boost.results <- assess.boost(bank_balanced.boost, test.bank_balanced)
boost.resultscv <- cv.boost(bank_balanced, k = 10)

# Random Forest
rf.results <- assess(bank_balanced.rf, test.bank_balanced)
rf.resultscv <- cv(bank_balanced, k = 10)
